...
def constructBayesNet(gameState):
    ...
    return bn.constructEmptyBayesNet(variables, edges, variableDomainsDict), obsVars
...
def fillObsCPT(bayesNet, gameState):
    ...
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            ...
            for i in obsVarFactor.getAllPossibleAssignmentDicts():
                p = 0
                blue = (i[obsVar] == BLUE_OBS_VAL)
                red =  (i[obsVar] == RED_OBS_VAL)
                none = (i[obsVar] == NO_OBS_VAL)
                housePositions = {TOP_LEFT_VAL: topLeftPos, BOTTOM_LEFT_VAL: bottomLeftPos, TOP_RIGHT_VAL: topRightPos,  BOTTOM_RIGHT_VAL: bottomRightPos}
                foodWall = obsPos in gameState.getHouseWalls(housePositions[i[FOOD_HOUSE_VAR]])
                ghostWall = obsPos in gameState.getHouseWalls(housePositions[i[GHOST_HOUSE_VAR]])
                
                if blue and foodWall:
                    p = 1 - PROB_FOOD_RED
                elif red and foodWall:
                    p = PROB_FOOD_RED
                elif none and foodWall:
                    p = 0
                elif blue and ghostWall:
                    p = 1 - PROB_GHOST_RED
                elif red and ghostWall:
                    p = PROB_GHOST_RED
                elif none and ghostWall:
                    p = 0
                elif red:
                    p = 0
                elif blue:
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    for assignment in factor.getAllPossibleAssignmentDicts():
        new = factor.getProbability(assignment)
        if new >= p:
            p = new
            bestMove = assignment
    return bestMove
